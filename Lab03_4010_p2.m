...
function main()
    ...
    %$$$$$$$$$$$$$$$$$$$4
    gyro_bias_error = (2*pi/180)^2;
    P(4,4) = gyro_bias_error;

    
    initial_gyro_bias = -pi/180;
    ...
    %#############
    Q = diag( [ (0.01)^2 ,(0.01)^2 , (1*pi/180)^2, 0;] ) ;  % Q(4,4) = 0 as bias doesnt change over time

    %############
    %*******  Pu -> covariance of the du (error between measured the actual state inputs)
    ...
    %#############
    InitSimulation(stdDevSpeed,stdDevGyro,sdev_rangeMeasurement,DtObservations,sdev_bearing, initial_gyro_bias);
        ...
        if nDetectedLandmarks>0,     % any laser data and detected landmarks?
            ...
            for u=1:nDetectedLandmarks
                ...
                H = [  -eDX/eDD , -eDY/eDD , 0, 0 ;
                    eDY/(eDD^2) , -eDX/(eDD^2), -1, 0];   % Jacobian of h(X);
                ...
                z  = [MeasuredRanges(u) - ExpectedRange ; wrapToPi(MeasuredBearings(u) - ExpectedAngle)];
                ...
                R = [sdev_rangeMeasurement*sdev_rangeMeasurement*4,0 ;
                    0                     ,4*deg2rad(sdev_bearing)^2];
...
function [ranges,bearing,IDs] = GetMeasurementsFomNearbyLandmarks(X,map)
    if map.nLandmarks>0,
        ...
        % return the landmark bearings 
        
        %############
        bearing = atan2(dy,dx) - X(3) + deg2rad(90);
...
% -------------------------------------------------------------------------
%#############
function InitSimulation(stdDevSpeed,stdDevGyro,sdev_rangeMeasurement,DtObservations,sdev_bearing, gyro_bias)
    global ContextSimulation;
    %#############
    ContextSimulation.Xreal = [ 0; 0;pi/2; 0] ;     % [x;y;phi]
    
    ContextSimulation.gyro_bias = gyro_bias;
    ...
    %############
    ContextSimulation.sdev_bearing = sdev_bearing;
return;
...
function [Noisy_speed,Noisy_GyroZ]=GetProcessModelInputs()
    ...
    %#############
    % so we don't add a bias to the real data
    % but add it to the noisy one
    % and so the EKF can detect that bias?
    % as in it notices it's off by 1 degree
    % btw change "bias error (deg/s)" to just "bias error (deg)"
    % "bias (deg)"*
    % doensn't make sense that
    % X_real's bias starts at 0
    Noisy_GyroZ =ContextSimulation.GyroZ+ContextSimulation.stdDevGyro*randn(1) - ContextSimulation.gyro_bias;
return;
...
function [nDetectedLandmarks,MeasuredRanges,MeasuredBearings,IDs]=GetObservationMeasurements(map)
    ...
    % simulated platform.
    [RealRanges,RealBearings,IDs] = GetMeasurementsFomNearbyLandmarks(ContextSimulation.Xreal,NavigationMap) ;
    % ...................................................
    nDetectedLandmarks = length(RealRanges) ;
    if (nDetectedLandmarks<1)       % no detected landmarks...
        
        %############
        MeasuredRanges=[];   IDs=[]; MeasuredBearings = [];    return ; 
    end;
    
    %############
    noiseinbearing = ContextSimulation.sdev_bearing*randn(size(RealBearings));
    MeasuredBearings = RealBearings + noiseinbearing; 
...
function SomePlots(Xreal_History,Xe_History,Xdr_History,map)
    ...
    figure(3) ; clf ; 
    Xe=Xe_History;
    subplot(411) ; plot(Xreal_History(1,:)-Xe(1,:)) ;ylabel('x-xe (m)') ;
    title('Performance EKF') ;
    subplot(412) ; plot(Xreal_History(2,:)-Xe(2,:)) ;ylabel('y-ye (m)') ;
    subplot(413) ; plot(180/pi*(Xreal_History(3,:)-Xe(3,:))) ;ylabel('heading error (deg)') ;
    %#########
    subplot(414) ; plot(180/pi*(Xreal_History(4,:)-Xe(4,:))) ;ylabel('bias error (deg/s)') ;
