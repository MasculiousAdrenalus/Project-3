...
%                     if dist < 0.4 
%                         OOI.kalm.id(num2) = ID.local.id(num2);
%                         OOI.kalm.x(num2) = ID.local.x(num2);
%                         OOI.kalm.y(num2) = ID.local.y(num2);
%                     end
%                 end
%             end
%            disp(OOI.kalm.id)
...
%                         disp(z)
                        ...
                        for foo2 = 1:length(OOI.local.x)
                            for foo1 = 1:length(OOI.global.x)
                                x_G = OOI.global.x(foo1);
                                y_G = OOI.global.y(foo1);
                                x_L = OOI.local.x(foo2);
                                y_L = OOI.local.y(foo2);
                                dist = sqrt((x_G - x_L)^2 + (y_G - y_L)^2);
                                if (dist <0.4)
                                    OOI.kalm.id(num2) = ID.local.id(num2);
                                    OOI.kalm.x(num2) = ID.local.x(num2);
