...
sdev_bearingMeasurement = 0.5 * pi/180; %std. of noise in bearing measurements 0.5 deg
   ...
   % The precition step, for this iteration, is done.
    ...
    if nDetectedLandmarks>0,     % any laser data and detected landmarks?
        ...
        for u=1:nDetectedLandmarks,
            ...
            eDT = atan2(eDY,eDX)-Xe(3)+(pi/2);
    
        
            % here is it. "H". I reuse some previous calculations.
            H = [  -eDX/eDD , -eDY/eDD , 0,0 ;eDY/(eDD^2), -eDX/(eDD^2), -1,0 ] ;   % Jacobian of h(X); size 2x3
            ...
            z  = [MeasuredRanges(u) - ExpectedRange; wrapToPi(MeasuredBearings(u) - ExpectedBearing)];    
...
function [ranges,bearings,IDs] = GetMeasurementsFomNearbyLandmarks(X,map)
    
    if map.nLandmarks>0,
        dx= map.landmarks(:,1) - X(1) ;
        dy= map.landmarks(:,2) - X(2) ;
        ranges = sqrt((dx.*dx + dy.*dy)) ;
        bearings = atan2(dx,dy)-X(3)+(pi/2);
...
function [speed,GyroZ] = SimuControl(X,t)
    ...
    GyroZ = 3*pi/180 + sin(0.1*2*pi*t/50)*.02 + X(4) ;         % some crazy driver moving the steering wheel...
return ;
...
function [nDetectedLandmarks,MeasuredRanges,MeasuredBearings,IDs]=GetObservationMeasurements(map)
    ...
    % simulated platform.
        ...
        noiseInBearing=ContextSimulation.sdev_bearingMeasurement*randn(size(RealBearings));
